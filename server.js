...
const serveStaticFile = (filename) => (req, res, next) => {
  ...
  if (fs.existsSync(filePath)) {
    ...
    res.status(200).send(`
      <!DOCTYPE html>
      <html>
      <head><title>Water Billing System</title></head>
      <body>
        <h1>🌊 Water Billing System</h1>
        <p>Server is running but static files are missing.</p>
        <p>Please ensure these files exist in the root directory:</p>
        <ul>
          <li>index.html</li>
          <li>login.html</li>
          <li>signup.html</li>
...
// ================== SERVE HTML PAGES (WITH FALLBACK) ==================
app.get('/', serveStaticFile('index.html'));
app.get('/login', serveStaticFile('login.html'));
app.get('/signup', serveStaticFile('signup.html'));
app.get('/admin', serveStaticFile('admin.html'));
app.get('/records', serveStaticFile('records.html'));
app.get('/main', serveStaticFile('main.html'));
...
app.post('/api/admin/approve-password-reset', protect, async (req, res) => {
  try {
    ...
    if (!resetRequest || resetRequest.status !== 'pending') return res.status(400).json({ error: 'Invalid request' });
    ...
    await sendEmail(user.email, 'Password Reset Approved', `<h2>New Temporary Password</h2><p>Your temporary password is: <strong>${newPassword}</strong></p><p>Please change it after logging in.</p>`);
...
app.post('/api/admin/reset-user-password', protect, async (req, res) => {
  try {
    ...
    await sendEmail(user.email, 'Password Reset', `<h2>New Password</h2><p>Your new temporary password is: <strong>${newPassword}</strong></p><p>Please change it after logging in.</p>`);
    res.json({ success: true });
  } catch (err) {
